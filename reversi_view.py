...
class ReversiView:
    ...
    def __init__(self, players, board_size=6, w=850, h=410):
        """
        ...
        self.possiblePlayers = {self.interractivePlayerName: -1}
    ...
    def draw_stone(self, x, y, color):
        ...
        """
        x_coord = (self.gridspacing * x) + \
            (1.0 - self.ovalDiamPart) * self.gridspacing
        y_coord = (self.gridspacing * y) + \
            (1.0 - self.ovalDiamPart) * self.gridspacing
    ...
    def draw_game_info_grid(self):
        ...
        """
        self.info = Canvas(self.root, height=self.h -
                           self.gridh, width=self.w - self.gridw)
        self.info.pack(side="left")

        label_stones = Label(
            self.info, text="Current stones:", font=("Helvetica", 10))
        label_stones.grid(row=1, column=0)
        label_max_time = Label(
            self.info, text="Max time:", font=("Helvetica", 10))
        label_max_time.grid(row=2, column=0)

        label_scale = Label(self.info, text='Game speed [ms]:',
        ...
        scale = Scale(self.info, variable=self.scale_var,
                      command=self.sleep_time_change_handler,
                      from_=0, to=1000, resolution=10,
                      width="15", orient=HORIZONTAL, length="225")
        scale.set(200)
        scale.grid(row=5, column=1, columnspan=3)

        self.button = Button(self.info, text="Play",
                             width="20", height="2",
        ...
        for i in range(2):
            ...
            self.labels_inform[i] = Label(self.info,
                                          ...
                                          font=("Helvetica", 10),
                                          foreground='black')
            self.labels_inform[i].grid(row=i + 3,
                                       column=0,
                                       columnspan=4)
            ...
            self.labels_player_name[i] = Label(self.info,
                                               ...
                                               font=("Helvetica", 12),
                                               foreground=self.colors[i])
            self.labels_player_name[i].grid(row=0,
                                            column=2 * i)
            ...
            self.option_menus[i].grid(row=0,
                                      column=2 * i + 1)
    ...
    def play_button_click_handler(self):
            ...
            for i in range(2):
                ...
                if self.option_menus_vars[i].get() == self.interractivePlayerName:
                    ...
                    if i == 0:
                        ...
                        self.game.player1 = player_class(self.game.player1_color,
                                                         self.game.player2_color)

                    else:
                        player_class = self.possiblePlayers[self.option_menus_vars[i].get(
                        )]
                        self.game.player2 = player_class(self.game.player2_color,
        ...
        if len(self.interactive_player_ids) != 0:
            if self.game_state == GameState.STOPPED:
                ...
                if (len(self.interactive_player_ids) == 1 and
                        self.interactive_player_ids[0] == 1):
    ...
    def place_stone_click_handler(self, event):
        ...
        if self.game_state != GameState.STOPPED and\
            ...
            pos_move = [int((event.y - self.offy) / self.gridspacing),
            ...
            if self.board.is_correct_move(pos_move, self.game.current_player_color):
                ...
                self.inform('incorrect move to %d %d' %
                            (pos_move[0], pos_move[1]), 'red')
